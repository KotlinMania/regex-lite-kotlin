# Claude Code Project Instructions — regex-lite-kotlin

## Project Overview

This is **regex-lite-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`regex-lite`](https://crates.io/crates/regex-lite).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/regex-lite/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/regex-lite>
